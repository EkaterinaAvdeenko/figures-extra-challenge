...
class Quadrilateral extends Figure {
  ...
  public Quadrilateral(Point a, Point b, Point c, Point d) {
    ...
    this.c = c;
    this.d = d;
    if (length(a, b) == 0 || length(b, c) == 0 || length(c, a) == 0 || length(a, d) == 0) {
      throw new IllegalArgumentException();
    }
    if (area() == 0) {
      throw new IllegalArgumentException();
    }
    if (checkDirectionalFactors(a, b, c)) {
      throw new IllegalArgumentException();
    }
    if (checkDirectionalFactors(b, c, d)) {
      throw new IllegalArgumentException();
    }
    double[][] points = {{a.getX(), a.getY()}, {b.getX(), b.getY()}, {c.getX(), c.getY()},
        {d.getX(), d.getY()}};
    if (!isConvex(points)) {
      throw new IllegalArgumentException();
    }
  ...
  public Point centroid() {
    ...
    return getIntersectionPoint(centroidOfTriangle1, centroidOfTriangle2, centroidOfTriangle3,
        centroidOfTriangle4);
  ...
  private boolean arePointsEqual(Quadrilateral A) {
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 4; j++) {
        if (roundTo3DecimalPlace(aPoints[i].getX()) == roundTo3DecimalPlace(bPoints[j].getX())
            && roundTo3DecimalPlace(aPoints[i].getY()) == roundTo3DecimalPlace(
            bPoints[j].getY())) {
          counter--;
        }
      }
